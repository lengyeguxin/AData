...
class IncomeCollector(BaseCollector):
    ...
    def _extract_values(self, item: Dict) -> tuple:
        """
        提取字段值（严格按照p2_schema.sql定义，完整38个字段）

        Args:
            item: API返回的单条数据

        Returns:
            字段值元组（38个字段，严格按照schema定义顺序）
        """
        return (
            # 基础字段（8个）
            item.get('ts_code'),
            ...
            item.get('fin_exp_int_income'),
            item.get('transfer_surplus_reserve'),

            # 其他（1个）
            item.get('transfer_risk_reserve'),
        )

    def _build_insert_query(self) -> str:
        """
        构建INSERT语句（ON CONFLICT处理，完整38个字段）
        ...
        fields = """ts_code, ann_date, f_ann_date, end_date, report_type, comp_type, end_type, update_flag,
            ...
            transfer_surplus_reserve, transfer_risk_reserve, updated_at"""

        # 构建VALUES占位符（38个 ? + NOW()）
        placeholders = ', '.join(['?'] * 38) + ', NOW()'
        ...
        update_fields = """f_ann_date = excluded.f_ann_date,
            ...
            operate_profit = excluded.operate_profit, total_cogs = excluded.total_cogs,
            interest_income = excluded.interest_income,
            profit_dedt = excluded.profit_dedt, sell_exp = excluded.sell_exp,
            admin_exp = excluded.admin_exp, fin_exp = excluded.fin_exp,
            asset_impair_loss = excluded.asset_impair_loss,
            non_oper_income = excluded.non_oper_income, non_oper_exp = excluded.non_oper_exp,
            total_profit = excluded.total_profit, income_tax = excluded.income_tax,
            ...
            oth_compr_income = excluded.oth_compr_income, t_compr_income = excluded.t_compr_income,
            compr_inc_attr_p = excluded.compr_inc_attr_p, compr_inc_attr_m_s = excluded.compr_inc_attr_m_s,
            ebit = excluded.ebit, ebitda = excluded.ebitda, rd_exp = excluded.rd_exp,
            fin_exp_int_exp = excluded.fin_exp_int_exp, fin_exp_int_income = excluded.fin_exp_int_income,
            transfer_surplus_reserve = excluded.transfer_surplus_reserve,
            transfer_risk_reserve = excluded.transfer_risk_reserve,
            updated_at = NOW()"""

        return f"""
            INSERT INTO income ({fields})
            VALUES ({placeholders})
