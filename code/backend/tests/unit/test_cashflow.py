...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "cashflow.json"
...
def test_collector_init():
    ...
    collector = CashflowCollector(db_path, api)

    assert collector.table_name == "cashflow"
    assert collector.api_name == "cashflow_vip"
    assert collector.vip_interface == True

    print(f"✅ CashflowCollector初始化测试通过")
