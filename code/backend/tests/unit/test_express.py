...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "express.json"
...
def test_collector_init():
    ...
    assert collector.table_name == "express"
    assert collector.api_name == "forecast_vip"
    assert collector.vip_interface == True

    print(f"✅ ExpressCollector初始化测试通过")
