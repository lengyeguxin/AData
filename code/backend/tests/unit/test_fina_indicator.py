...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "fina_indicator.json"
...
def test_collector_init():
    ...
    collector = FinaIndicatorCollector(db_path, api)

    assert collector.table_name == "fina_indicator"
    assert collector.api_name == "fina_indicator_vip"
    assert collector.vip_interface == True

    print(f"✅ FinaIndicatorCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("fina_indicator表Collector单元测试")
    print("=" * 80)
