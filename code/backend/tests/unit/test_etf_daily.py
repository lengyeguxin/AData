...
import json
from src.collectors.etf_daily_collector import ETFDailyCollector
...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "etf_daily.json"
...
def test_collector_init():
    ...
    collector = ETFDailyCollector(db_path, api)

    assert collector.table_name == "etf_daily"
    assert collector.api_name == "fund_daily"

    print(f"✅ ETFDailyCollector初始化测试通过")
