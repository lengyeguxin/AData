"""
stock_daily_basic表Collector单元测试
...
import json
from src.collectors.stock_daily_basic_collector import StockDailyBasicCollector
...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "stock_daily_basic.json"
...
def test_collector_init():
    ...
    collector = StockDailyBasicCollector(db_path, api)

    assert collector.table_name == "stock_daily_basic"
    assert collector.api_name == "daily_basic"

    print(f"✅ StockDailyBasicCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("stock_daily_basic表Collector单元测试")
    print("=" * 80)
