...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "express_brief.json"
...
def test_collector_init():
    ...
    assert collector.table_name == "express_brief"
    assert collector.api_name == "express_vip"
    assert collector.vip_interface == True

    print(f"✅ ExpressBriefCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("express_brief表Collector单元测试")
    print("=" * 80)
