...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "stock_monthly.json"
...
def test_collector_init():
    ...
    collector = MonthlyCollector(db_path, api)

    assert collector.table_name == "stock_monthly"
    assert collector.api_name == "stk_week_month_adj"

    print(f"✅ MonthlyCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("stock_monthly表Collector单元测试")
    print("=" * 80)
