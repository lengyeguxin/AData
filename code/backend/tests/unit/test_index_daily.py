...
import json
from src.collectors.index_daily_collector import IndexDailyCollector
...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "index_daily.json"
...
def test_collector_init():
    ...
    collector = IndexDailyCollector(db_path, api)

    assert collector.table_name == "index_daily"
    assert collector.api_name == "index_daily"

    print(f"✅ IndexDailyCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("index_daily表Collector单元测试")
    print("=" * 80)
