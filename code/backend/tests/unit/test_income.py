...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "income.json"
...
def test_collector_init():
    ...
    assert collector.table_name == "income"
    assert collector.api_name == "income_vip"
    assert collector.vip_interface == True

    print(f"✅ IncomeCollector初始化测试通过")
