...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "balancesheet.json"
...
def test_collector_init():
    ...
    collector = BalancesheetCollector(db_path, api)

    assert collector.table_name == "balancesheet"
    assert collector.api_name == "balancesheet_vip"
    assert collector.vip_interface == True

    print(f"✅ BalancesheetCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("balancesheet表Collector单元测试")
    print("=" * 80)
