...
# Mock数据
mock_data_file = Path(__file__).parent.parent.parent.parent.parent / "tests" / "mock_data" / "stock_weekly.json"
...
def test_collector_init():
    ...
    collector = WeeklyCollector(db_path, api)

    assert collector.table_name == "stock_weekly"
    assert collector.api_name == "stk_week_month_adj"

    print(f"✅ WeeklyCollector初始化测试通过")
...
if __name__ == "__main__":
    print("=" * 80)
    print("stock_weekly表Collector单元测试")
    print("=" * 80)
